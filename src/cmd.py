...
def testdl(session, atoms):
    ...
    os.chdir(os.path.expanduser("~/Desktop/test_plugin/ChimeraX"))
    logging.basicConfig(filename="testdl.log", level=logging.DEBUG)
    logging.info("Starting to run testdl")
    logging.info("Current path: ", root)
    t_i = time()

    # os.chdir(os.path.expanduser(os.path.abspath('..')))
    class Args:
        n_memories = 0
        memory_strength = 0.5
        cuda = "yes"
        seed = 0
        test_pred_path = "./output/labels/"
        test_vis_path = "./output/visuals/"
        map_path = "./data/EMDBdata_6/"
        # test_pred_path = "./output/testing_predictions/labels/"
        # test_vis_path = "./output/testing_predictions/visuals/"
        # map_path = "./data/EMDBdata_6/"
        n_tasks = 3
    ...
    print("loading data...")
    test_data = test.load_datasets(args)

    Model = importlib.import_module(".model.unet", package="chimerax.dl_struct")
    model = Model.Gem_UNet(args)

    test.test_model(model, test_data, args)
    t_f = time()
    print(("{0} s total usage...").format(t_f-t_i))
    logging.info(("{0} s to evaluate total.").format(
        t_f - t_i,))


testdl_desc = CmdDesc(required=[("atoms", Or(AtomsArg, EmptyArg))])
...
def cofm(session, atoms, weighted=False, transformed=True):
    ...
    session.logger.info(
        "%s center of mass: %s" % ("weighted" if weighted else "unweighted", cofm)
    )


cofm_desc = CmdDesc(
    required=[("atoms", Or(AtomsArg, EmptyArg))],
    optional=[("weighted", BoolArg), ("transformed", BoolArg)],
)
...
highlight_desc = CmdDesc(
    required=[("atoms", Or(AtomsArg, EmptyArg)), ("color", ColorArg)],
    optional=[
        ("weighted", BoolArg),
        ("transformed", BoolArg),
